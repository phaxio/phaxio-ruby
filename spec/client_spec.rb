  ...
  describe 'making a request' do
    it 'uses the configured API key and secret by default' do
      expect(RestClient).to receive(:get) do |_url, rest_client_params|
        expect(rest_client_params[:api_key]).to eq(Phaxio.api_key)
        expect(rest_client_params[:api_secret]).to eq(Phaxio.api_secret)
      end
    ...
    describe 'GET requests' do
      it 'sends the request to Phaxio' do
        ...
        expect(RestClient).to receive(:get) do |url, rest_client_params|
          ...
          expect(rest_client_params[:page]).to eq(params[:page])
          expect(rest_client_params[:per_page]).to eq(params[:per_page])
        end
